// pages/Contact.jsx - Full i18n Integration
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.message) {
      setError(t('contact.fillRequired', 'Please fill in all required fields'));
      setLoading(false);
      return;
    }

    if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(formData.email)) {
      setError(t('contact.invalidEmail', 'Please provide a valid email address'));
      setLoading(false);
      return;
    }
    ...
    try {
      try {
        ...
        setSuccessMessage(t('contact.successMessage', "Message sent successfully! We'll get back to you soon."));
      }
      
      setSuccess(true);
      setFormData({ name: '', email: '', phone: '', subject: '', message: '' });
    } catch (err) {
      setError(t('contact.somethingWrong', 'Something went wrong. Please try again.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const contactCards = [
    {
      ...
      details: t('contact.address', '123 Avenue Habib Bourguiba\nTunis 1001, Tunisia'),
    },
    {
    ...
    },
    {
      ...
      details: t('contact.hours', 'Mon - Sat | 9AM - 7PM'),
    },
  ];

  return (
    <Box sx={{ bgcolor: '#ffffff', direction: isRTL ? 'rtl' : 'ltr' }}>
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
        <Grid container spacing={5}>
          {/* Contact Form */}
          <Grid size={{ xs: 12, md: 7 }}>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <Typography sx={{ fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', fontSize: '1.6rem', fontWeight: 700, color: '#141010', mb: 0.5, textAlign: isRTL ? 'right' : 'left' }}>
                {t('contact.sendMessage', 'Send a Message')}
              </Typography>
              <Typography sx={{ fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', fontSize: '0.9rem', color: '#888', mb: 3, textAlign: isRTL ? 'right' : 'left' }}>
                {t('contact.formDesc', "Fill the form and we'll respond within 24 hours")}
              </Typography>

              {error && (
                <Alert severity="error" sx={{ mb: 3, borderRadius: '12px', fontFamily: isRTL ? '"Noto Kufi Arabic"' : 'Amaranth, sans-serif' }} onClose={() => setError('')}>
                  {error}
                </Alert>
              )}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField 
                      ...
                      onChange={handleChange} 
                      placeholder={t('contact.subjectPlaceholder', 'Order inquiry')} 
                      sx={textFieldStyle(isRTL)}
                  ...
                  <Grid size={{ xs: 12 }}>
                    <Button 
                      ...
                      fullWidth 
                      disabled={loading} 
                      endIcon={isRTL ? null : <Send />}
                      startIcon={isRTL ? <Send /> : null}
                      sx={{
                        py: 1.6, bgcolor: '#141010', color: '#ffffff', 
                        fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', 
                        fontWeight: 700, fontSize: '1rem', textTransform: 'none', borderRadius: '14px',
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <motion.div initial={{ opacity: 0, x: isRTL ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}>
              <Typography sx={{ fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', fontSize: '1.6rem', fontWeight: 700, color: '#141010', mb: 3, textAlign: isRTL ? 'right' : 'left' }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {contactCards.map((card, idx) => (
                  <motion.div key={idx} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 * idx }} whileHover={{ y: -3 }}>
                    <Box sx={{ display: 'flex', gap: 2, p: 2.5, borderRadius: '16px', border: '1px solid rgba(0,0,0,0.06)', flexDirection: isRTL ? 'row-reverse' : 'row', transition: 'all 0.3s ease', '&:hover': { boxShadow: '0 6px 25px rgba(0,0,0,0.06)' } }}>
                      ...
                      <Box sx={{ textAlign: isRTL ? 'right' : 'left' }}>
                        <Typography sx={{ fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', fontSize: '0.95rem', fontWeight: 700, color: '#141010', mb: 0.3 }}>
                          {card.title}
                        </Typography>
                        <Typography sx={{ fontFamily: isRTL ? '"Noto Kufi Arabic", "Tajawal", sans-serif' : 'Amaranth, sans-serif', fontSize: '0.85rem', color: '#888', whiteSpace: 'pre-line', lineHeight: 1.5 }}>
                          {card.details}
                        </Typography>
                      </Box>
                    </Box>
