...
const FETCH_CONFIG = {
  limit: 50,
  status: 'active',  // This works because the offer model has 'status' field
  sortBy: 'createdAt',
  sortOrder: 'desc', 
};
...
const TOKEN = {
  ...
  white: '#FFFFFF',
  offWhite: '#F7F6F3',
  accent: '#FF6B2B',          // hot orange for offers
  accentDark: '#E55A1E',
...
const formatOfferData = (offer) => ({
  ...
  // Use offer's pre-calculated fields
...
const getTimeRemaining = (endDate) => {
  ...
  if (hours > 0) return { text: `${hours}h ${minutes}m left`, expired: false, urgent: hours < 6 };
  return { text: `${minutes}m left`, expired: false, urgent: true };
};

const initializeDefaultSizes = (offers) => {
  const map = {};
  offers.forEach((o) => {
    if (o.sizes?.length) map[o.id] = o.sizes[0];
  });
  return map;
};
  ...
  const fetchOffers = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const formatted = response.data.data.map(formatOfferData);
        setOffers(formatted);
        return formatted;
      }
...
const ProductSkeleton = ({ index }) => (
  ...
  >
    <Box sx={{ borderRadius: '16px', overflow: 'hidden', bgcolor: TOKEN.offWhite }}>
      ...
      <Box sx={{ p: 2 }}>
        {[80, 55, 40].map((w, i) => (
          <Box key={i} className="shimmer-bone" sx={{ height: 12, width: `${w}%`, borderRadius: 4, mb: 1 }} />
        ))}
  ...
  return (
    <Box
      sx={{
        ...
        bottom: 12,
        left: 12,
        bgcolor: time.urgent ? TOKEN.accent : TOKEN.black,
        color: TOKEN.white,
        fontFamily: TOKEN.fontDisplay,
        fontWeight: 700,
        fontSize: '0.62rem',
...
const ProductRating = ({ rating, salesCount }) => (
  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.75 }}>
    ...
    />
    <Typography sx={{ fontFamily: TOKEN.fontBody, fontSize: '0.65rem', color: TOKEN.muted, lineHeight: 1 }}>
      {(rating || 0).toFixed(1)}
    </Typography>
    {salesCount > 0 && (
      <Typography sx={{ fontFamily: TOKEN.fontBody, fontSize: '0.6rem', color: TOKEN.muted, ml: 'auto' }}>
...
const ProductPrice = ({ mainPrice, discountedPrice, discount }) => (
...
const OfferCard = ({ offer, selectedSize, onOfferClick, onAddToCart, onSizeSelect, onCopyPromo, index }) => (
  <motion.div
  ...
  >
    ...
    >
      ...
      <Box sx={{ position: 'relative', overflow: 'hidden', pt: '110%', bgcolor: TOKEN.offWhite }}>
        ...
        <Box
          ...
          sx={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(to top, rgba(10,10,10,0.55) 0%, rgba(10,10,10,0.1) 45%, transparent 70%)',
            opacity: 0,
            pointerEvents: 'none',
          }}
        />
        ...
        <TimerBadge endDate={offer.endDate} />
      </Box>
      ...
      <CardContent sx={{ p: { xs: 1.75, sm: 2.25 }, pb: '16px !important', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <ProductRating rating={offer.review} salesCount={offer.salesCount} />
        <ProductPrice mainPrice={offer.mainPrice} discountedPrice={offer.discountedPrice} discount={offer.discount} />

        <SizeSelector
          sizes={offer.sizes}
          selectedSize={selectedSize}
          onSizeSelect={(size) => (e) => { e.stopPropagation(); onSizeSelect(offer.id, size); }}
        />
        ...
        <Button
          ...
          sx={{
            ...
            '&:hover': { bgcolor: '#1f1f1f', transform: 'translateY(-1px)' },
...
const ErrorState = ({ error, onRetry }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    <Box sx={{ textAlign: 'center', py: 8 }}>
      <Typography sx={{ fontFamily: TOKEN.fontDisplay, fontWeight: 800, fontSize: '1.6rem', color: '#c0392b', mb: 1, textTransform: 'uppercase', letterSpacing: '-0.01em' }}>
        Something went wrong
      </Typography>
      <Typography sx={{ fontFamily: TOKEN.fontBody, color: TOKEN.muted, mb: 3, fontSize: '0.88rem' }}>{error}</Typography>
      <Button onClick={onRetry} variant="contained" disableElevation
        sx={{ fontFamily: TOKEN.fontDisplay, fontWeight: 800, fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', bgcolor: TOKEN.black, color: TOKEN.white, borderRadius: '10px', px: 4, py: 1.2, '&:hover': { bgcolor: '#1f1f1f' } }}>
        Retry
      </Button>
    </Box>
  </motion.div>
...
const Offers = () => {
  ...
  const handleOfferClick = useCallback(
    (offer) => navigate(`/product/${offer.id}`, { state: { product: offer } }),
    [navigate]
  );
  ...
  const handleAddToCart = useCallback(
    (offer, size, event) => {
      ...
      addToCart(offer, 1, s);
      showSnackbar(`${offer.name} — size ${s} added`, 'success');
    },
    [addToCart, showSnackbar]
  );

  const handleSizeSelect = useCallback(
    (offerId, size) => setSelectedSize((prev) => ({ ...prev, [offerId]: size })),
    []
  );

  const handleCopyPromo = useCallback((code) => {
    navigator.clipboard.writeText(code).then(() => {
      showSnackbar(`Promo code "${code}" copied!`, 'success');
    }).catch(() => {
            ...
            <AnimatePresence>
              {offers.map((offer, index) => (
                <Grid size={{ xs: 6, sm: 6, md: 4 }} key={offer.id}>
                  <OfferCard
                    offer={offer}
                    selectedSize={selectedSize[offer.id]}
